"""
JARVIS Screen Awareness — see what's on the user's screen (macOS only).

Two capabilities:
1. Window/app list via AppleScript (fast, text-based)
2. Screenshot via screencapture → Claude vision API (sees everything)

On non-macOS systems all functions return empty/unavailable results.
"""
...
async def get_active_windows() -> list[dict]:
    """Get list of visible windows (macOS only).
...
async def get_running_apps() -> list[str]:
    """Get list of running application names (macOS only)."""
    if not IS_MACOS:
        return []
    script = """
tell application "System Events"
...
async def take_screenshot(display_only: bool = True) -> str | None:
    """Take a screenshot and return base64-encoded PNG (macOS only).
