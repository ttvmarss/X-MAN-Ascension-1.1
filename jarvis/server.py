...
# Desktop path — falls back to home dir on non-macOS / cloud
...
- You CAN open a terminal and run commands on the host system
- You CAN open a web browser and browse any URL or search query
- You CAN spawn Claude Code for coding tasks
- You CAN create project folders and manage files
- You CAN check local projects and their git status
- You CAN plan complex tasks by asking smart questions before executing
- You CAN see what's on {user_name}'s screen — open windows, active apps, and screenshot vision (macOS only)
- You CAN read {user_name}'s calendar — today's events, upcoming meetings, schedule overview (macOS only)
- You CAN read {user_name}'s email (READ-ONLY) — unread count, recent messages, search by sender/subject (macOS only)
- You CAN read and create Notes (macOS only)
...
- You CAN control {user_name}'s Windows PC when the Windows agent is connected (remote commands, file management)
- You ARE accessible from anywhere in the world — iPhone, PC, or any browser — through the cloud
...
async def fetch_weather() -> str:
    ...
    try:
        ...
        url = f"https://wttr.in/{location}?format=%l:+%C,+%t" if location else "https://wttr.in/?format=%l:+%C,+%t"
        async with httpx.AsyncClient(timeout=5.0) as http:
            resp = await http.get(url, headers={"User-Agent": "curl"})
...
class ClaudeTaskManager:
    ...
    async def _run_task(self, task: ClaudeTask):
        ...
        if work_dir == "." or not work_dir:
            # Create a new project folder — Desktop or home dir
        ...
        cmd_str = f"cd {work_dir!r} && cat .jarvis_prompt.md | claude -p --dangerously-skip-permissions | tee .jarvis_output.txt; echo '\\n--- JARVIS TASK COMPLETE ---'"

        if IS_MACOS:
            # Open Terminal.app with claude running visibly
            applescript = (
                'tell application "Terminal"\n'
                '    activate\n'
                f'    do script {json.dumps(cmd_str)}\n'
                'end tell'
            )
            ...
            # Cloud/Linux: run directly as a background subprocess
            import shlex
...
async def _focus_terminal_window(project_name: str):
    """Bring a Terminal window matching the project name to front (macOS only)."""
    if not IS_MACOS:
        return
...
def _find_project_dir(project_name: str) -> str | None:
    """Find a project directory by name from cached projects or Desktop/home."""
    for p in cached_projects:
        if project_name.lower() in p.get("name", "").lower():
            return p.get("path")
    for search_dir in [DESKTOP_PATH, Path.home()]:
...
def _refresh_context_sync():
    ...
    def _worker():
        ...
        while True:
            try:
                # Screen — macOS only via AppleScript
                if IS_MACOS:
                    try:
...
'''],
            ...
            # Weather — refresh every 30s using wttr.in (location-aware)
            try:
                location = WEATHER_LOCATION.strip().replace(" ", "+")
                weather_url = f"https://wttr.in/{location}?format=%l:+%C,+%t" if location else "https://wttr.in/?format=%l:+%C,+%t"
                req = urllib.request.Request(weather_url, headers={"User-Agent": "curl"})
                with urllib.request.urlopen(req, timeout=5) as resp:
                    text = resp.read().decode().strip()
                    if text:
                        _ctx_cache["weather"] = f"Current weather: {text}"
            except Exception:
                pass
...
# Optional token authentication — only active when AUTH_TOKEN is set in .env
...
async def auth_middleware(request: Request, call_next):
    if AUTH_TOKEN:
        ...
        if token != AUTH_TOKEN:
            return _FResponse(content='{"error":"Unauthorized"}', status_code=401, media_type="application/json")
    return await call_next(request)
...
async def handle_build(target: str) -> str:
    ...
    cmd_str = f"cat .jarvis_prompt.txt | claude -p --dangerously-skip-permissions"

    if IS_MACOS:
        script = (
            'tell application "Terminal"\n'
            "    activate\n"
            f'    do script "cd {path} && {cmd_str}"\n'
            "end tell"
        )
...
async def handle_show_recent() -> str:
    ...
    if IS_MACOS:
        ...
        return f"Project is at {last['path']}, sir."
...
# ---------------------------------------------------------------------------
# Windows PC Agent WebSocket endpoint
# ---------------------------------------------------------------------------

_pc_agents: dict[str, WebSocket] = {}  # hostname -> WebSocket


@app.websocket("/ws/pc")
async def pc_agent_handler(ws: WebSocket):
    """WebSocket endpoint for the Windows PC agent (windows_agent.py)."""
    await ws.accept()
    agent_id = None
    log.info("PC agent connecting...")
    try:
        async for raw in ws.iter_text():
            ...
            if msg_type == "agent_hello":
                ...
                agent_id = system.get("hostname", str(id(ws)))
                _pc_agents[agent_id] = ws
                log.info(f"PC agent connected: {agent_id} ({system.get('platform')})")
                await ws.send_json({"type": "hello_ack", "message": "JARVIS connected."})

            elif msg_type == "pc_result":
                log.info(f"PC result for command {msg.get('command_id')}: {msg.get('result', {}).get('success')}")
                # Broadcast result to all voice WebSocket clients
                result = msg.get("result", {})
                for voice_ws in task_manager._websockets:
                    try:
                        await voice_ws.send_json({"type": "pc_result", "result": result})
                    except Exception:
                        pass
        ...
        log.info(f"PC agent disconnected: {e}")
    finally:
        if agent_id and agent_id in _pc_agents:
            del _pc_agents[agent_id]
            log.info(f"PC agent removed: {agent_id}")


@app.post("/api/pc/command")
async def send_pc_command(req: dict):
    """Send a command to the connected Windows PC agent."""
    if not _pc_agents:
        return JSONResponse({"error": "No PC agent connected"}, status_code=503)
    import uuid as _uuid
    command_id = str(_uuid.uuid4())[:8]
    payload = {**req, "type": "pc_command", "command_id": command_id}
    # Send to first available agent
    for agent_ws in _pc_agents.values():
        try:
            await agent_ws.send_json(payload)
            return {"command_id": command_id, "sent": True}
        except Exception as e:
            return JSONResponse({"error": str(e)}, status_code=500)
    return JSONResponse({"error": "No PC agent available"}, status_code=503)


@app.get("/api/pc/status")
async def pc_status():
    """Check if a Windows PC agent is connected."""
    return {
        "connected": len(_pc_agents) > 0,
        "agents": list(_pc_agents.keys()),
    }
...
async def voice_handler(ws: WebSocket):
    ...
    # Response cancellation — use a mutable dict so closures share state
    _state = {
        "response_id": 0,    # Incremented each time user speaks
    ...
    try:
        ...
        while True:
            ...
            # Cancel any in-flight response — barge-in support
            _state["response_id"] += 1
            my_response_id = _state["response_id"]
            _state["cancel"] = True
            # Signal the frontend to stop playing audio immediately
            try:
                await ws.send_json({"type": "stop_audio"})
            except Exception:
                pass
            await asyncio.sleep(0.05)  # Let in-flight coroutines see the cancel flag
            _state["cancel"] = False
            ...
            try:
                ...
                # TTS — abort if user already spoke again (barge-in)
                if _state["cancel"] or _state["response_id"] != my_response_id:
                    log.info("Response cancelled — user spoke before TTS")
                    await ws.send_json({"type": "status", "state": "idle"})
                    continue
...
async def api_fix_self():
    ...
    if IS_MACOS:
        ...
        # On Linux/cloud, spawn in a new terminal emulator if available
        for term in ["xterm", "gnome-terminal", "bash"]:
            import shutil as _sh
            if _sh.which(term):
                await asyncio.create_subprocess_exec(
                    term, "-e", f"bash -c 'cd {jarvis_dir} && claude --dangerously-skip-permissions; exec bash'",
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE,
                )
                break
