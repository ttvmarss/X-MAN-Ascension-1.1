"""
JARVIS Action Executor — system actions with macOS/cross-platform support.
...
# Desktop path — falls back to home on Linux/cloud
DESKTOP_PATH = Path.home() / "Desktop"
if not DESKTOP_PATH.exists():
    DESKTOP_PATH = Path.home()

_NOT_SUPPORTED = {"success": False, "confirmation": "That action requires macOS, sir."}
...
async def open_terminal(command: str = "") -> dict:
    ...
    if IS_MACOS:
        ...
        # Linux/cloud: try common terminal emulators
        for term in ["gnome-terminal", "xterm", "konsole", "bash"]:
            if shutil.which(term):
                cmd = [term]
                if command:
                    cmd += (["--"] if term == "gnome-terminal" else ["-e"]) + [command]
                proc = await asyncio.create_subprocess_exec(
                    *cmd,
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE,
        ...
        return {"success": False, "confirmation": "No terminal emulator found on this system, sir."}


async def open_browser(url: str, browser: str = "chrome") -> dict:
    """Open URL in user's browser (Chrome, Firefox, or system default)."""
    if IS_MACOS:
        ...
        # Linux/cloud: use xdg-open or available browser
        for cmd in ["xdg-open", "google-chrome", "chromium-browser", "firefox", "sensible-browser"]:
            if shutil.which(cmd):
                proc = await asyncio.create_subprocess_exec(
                    cmd, url,
                    stdout=asyncio.subprocess.PIPE,
                ...
                return {"success": True, "confirmation": f"Opened {url} in browser, sir."}
        log.info(f"No browser available on this system, URL: {url}")
        return {"success": False, "confirmation": f"No browser available, sir. URL: {url}"}
...
async def open_claude_in_project(project_dir: str, prompt: str) -> dict:
    """Open a terminal in the project dir and run Claude Code interactively."""
    # Write prompt to CLAUDE.md — claude reads this automatically on startup
    ...
    if IS_MACOS:
        script = (
            'tell application "Terminal"\n'
            "    activate\n"
            f'    do script "cd {project_dir} && {claude_path} --dangerously-skip-permissions"\n'
            "end tell"
        )
        proc = await asyncio.create_subprocess_exec(
            "osascript", "-e", script,
        ...
        # Cloud/Linux: run directly as subprocess
        proc = await asyncio.create_subprocess_exec(
        ...
        return {"success": True, "confirmation": "Claude Code is running in the background, sir."}


async def prompt_existing_terminal(project_name: str, prompt: str) -> dict:
    """Find a Terminal window matching a project name and type a prompt into it (macOS only)."""
    if not IS_MACOS:
